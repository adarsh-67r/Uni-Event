...
export default function EventChatScreen({ route, navigation }) {
  ...
  useEffect(() => {
    ...
    const q = query(collection(db, 'events', eventId, 'messages'), orderBy('createdAt', 'desc'));
    ...
    return (
      <View style={[styles.messageRow, isMe ? styles.myMessageRow : styles.otherMessageRow]}>
        ...
        >
          ...
          <Text
            style={[styles.messageText, isMe ? { color: '#fff' } : { color: theme.colors.text }]}
          >
            {item.text}
          </Text>
          <Text
      ...
      >
        ...
        >
          ...
          <TouchableOpacity
            style={[
              styles.sendBtn,
              { backgroundColor: theme.colors.primary, opacity: !inputText.trim() ? 0.5 : 1 },
            ]}
            onPress={handleSend}
