...
export default function AttendanceDashboard({ route, navigation }) {
  ...
  useEffect(() => {
    const q = query(collection(db, 'events', eventId, 'checkIns'), orderBy('checkedInAt', 'desc'));
  ...
  const StatCard = ({ icon, label, value, color, subtitle, gradient }) => (
    <View style={styles.statCard}>
      <LinearGradient colors={gradient || [color + '20', color + '10']} style={styles.statGradient}>
    ...
    return (
      <View style={[styles.checkInItem, { backgroundColor: theme.colors.surface }]}>
        ...
        <View style={styles.checkInInfo}>
          <Text style={[styles.checkInName, { color: theme.colors.text }]}>{item.userName}</Text>
        ...
        <View style={styles.checkInTime}>
          ...
          <Text style={[styles.timeText, { color: theme.colors.textSecondary }]}>{timeAgo}</Text>
        </View>
      </View>
    );
  };
          ...
          return (
            <View key={key} style={styles.analyticsItem}>
              <View style={styles.analyticsItemHeader}>
                <Text style={[styles.analyticsLabel, { color: theme.colors.text }]}>{key}</Text>
    ...
    >
      ...
      <ScrollView showsVerticalScrollIndicator={false}>
        ...
        {Object.keys(departmentStats).length > 0 && (
          <AnalyticsSection title="Department Breakdown" data={departmentStats} icon="school" />
        )}

        {Object.keys(yearStats).length > 0 && (
          <AnalyticsSection title="Year-wise Distribution" data={yearStats} icon="calendar" />
        )}
            ...
            >
              {eventData?.feedbackRequestSent ? (
                <>
                  ...
                  <Text style={[styles.exportBtnText, { color: theme.colors.success }]}>
                    Feedback Sent
                  </Text>
                </>
              ) : (
        ...
        <View style={styles.exportContainer}>
          ...
          <View style={styles.exportButtons}>
            ...
            <TouchableOpacity
              ...
              onPress={
                eventData?.hasCustomForm ? handleExportFormResponses : handleExportParticipants
              }
              disabled={exporting}
            >
        ...
        <View style={styles.modalOverlay}>
          <View style={[styles.modalContent, { backgroundColor: theme.colors.surface }]}>
            ...
            <View style={{ paddingVertical: 20 }}>
              ...
              />
              <Text
                style={[styles.modalDescription, { color: theme.colors.text, textAlign: 'center' }]}
              >
              ...
              >
                They will receive a beautiful email with a link to rate the event and provide
                feedback.
              </Text>
            </View>
