...
export default function FormBuilderScreen({ navigation, route }) {
  ...
  const addField = () => {
    ...
    const newField = {
      ...
      options: newFieldType === 'dropdown' ? newFieldOptions.split(',').map(o => o.trim()) : [],
    };
            ...
            <View key={field.id} style={styles.fieldCard}>
              ...
              <View style={styles.controls}>
                <TouchableOpacity onPress={() => moveField(index, 'up')} disabled={index === 0}>
                  <Ionicons name="chevron-up" size={24} color={index === 0 ? '#333' : '#666'} />
                </TouchableOpacity>
                <TouchableOpacity
            ...
            <ScrollView contentContainerStyle={{ padding: 20 }}>
              ...
              >
                {FIELD_TYPES.map(t => (
                  <TouchableOpacity
                    key={t.value}
                    style={[styles.typeChip, newFieldType === t.value && styles.typeChipActive]}
                    onPress={() => setNewFieldType(t.value)}
                  >
                    <Ionicons
                      name={t.icon}
                      size={18}
                      color={newFieldType === t.value ? '#fff' : theme.colors.text}
                    />
                    <Text
                      style={[styles.typeText, newFieldType === t.value && styles.typeTextActive]}
                    >
                      {t.label}
                    </Text>
                  </TouchableOpacity>
